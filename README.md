...
- Поддержка автоматического и ручного выбора портов
...
``` C
// Глобальные настройки (можно переопределить аргументами)
char HOST_IP[16] = "127.0.0.1";    // IP-адрес сервера
in_port_t PORT = 9090;             // Порт сервера
in_port_t CLIENT_PORT = 0;         // Порт клиента (0 = авто-выбор)

// Константы
#define MAX_PAYLOAD_SIZE 1024      // Макс. размер сообщения
#define MAX_BUFFER_SIZE 2048       // Размер буфера
#define MIN_EPHEMERAL_PORT 1024    // Минимальный порт для авто-выбора
#define MAX_EPHEMERAL_PORT 65535   // Максимальный порт для авто-выбора
```

### Raw Socket особенности

- Использует `AF_INET, SOCK_RAW, IPPROTO_UDP` вместо `AF_PACKET`
- Руководство IP-заголовками (IP_HDRINCL)
- Кастомная реализация расчета контрольной суммы
- Обработка на уровне IP пакетов (без Ethernet заголовков)
...
sudo ./build/server 192.168.1.100 8080
...
```
# По умолчанию (автоматический порт клиента)
sudo ./build/client

# С указанием сервера
sudo ./build/client 192.168.1.100 8080

# С указанием сервера и фиксированного порта клиента
sudo ./build/client 192.168.1.100 8080 12345

# Автоматический порт клиента к другому серверу
sudo ./build/client 192.168.1.100 8080 0
